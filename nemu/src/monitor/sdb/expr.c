...
static bool make_token(char *e) {
  ...
  while (e[position] != '\0') {
    ...
    for (i = 0; i < NR_REGEX; i++) {
      Assert(
          (nr_token <= 65536),
          "tokens[65536] is full"); // prevent the length of digit is too long
      ...
      if (regexec(&re[i], e + position, 1, &pmatch, 0) == 0 &&
        ...
        case TK_REG: //	for register, record the name of the register and turn
                     // into str
          tokens[nr_token].type = rules[i].token_type;
...
bool check_parentheses(int p, int q) {
  ...
  Assert(tokens[p].type != ')',
         "wrong parenthese, starting with ')', instead of '('");

  if (tokens[p].type == '(') {
    //	in case: "()"
    if (tokens[q].type == ')' && q == p + 1)
      return true;
