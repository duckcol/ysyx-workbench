...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                   \
  ...
  INSTPAT(
      ...
      add_ftrace(s->dnpc, 0); //  add ftrace
  );
  ...
  INSTPAT(
      ...
      R(rd) = s->snpc;                       //  store next pc
      s->dnpc = (src1 + imm) & ~((word_t)1); // jmp to target
      add_ftrace(s->dnpc, src1 == R(1));     // ftrace and differ ret and jmp
  );
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall, N,
          //  m mode ecall's NO is 11(decimal, hex is 0xb)
          word_t target = isa_raise_intr(0xb, s->pc);
